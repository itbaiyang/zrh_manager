...
detailAppCtrl.controller('DetailAppCtrl',
    ...
    $scope.editApply = function (id) {
        $state.go('apply.edit_apply', {'id': id})
    };  //编辑页面
    $scope.apply_help = function (id) {
        $state.go('apply.apply_help', {'id': id})
    }; //帮助申请
    $scope.change_bank = function (id, mobile) {
        $state.go('apply.change_bank', {'id': id, 'mobile': mobile});
    }; //更改银行
    $scope.messages = function (id) {
        $state.go('apply.message', {'id': id});
    };   //留言板
    $scope.distribute = function (id) {
        $state.go('apply.distribute', {'id': id});
    };//递交资料
    $scope.change_register = function (id) {
        $state.go('apply.change_register', {'id': id});
    };//更改银行
    $scope.choice_sale = function (id) {
        $state.go('apply.choice_sale', {'id': id});
    }; //选择销售人员
